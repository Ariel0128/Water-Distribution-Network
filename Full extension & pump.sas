proc import out=demand
	...
	replace;
run; 
proc import out=demand_lockdown
	datafile = "/home/u47247130/sasuser.v94/optimisation/demand_lockdown.csv"
	replace;
run; 
...
proc optmodel ;
	...
	read data demand_lockdown into 
	DESTINATIONS=[Destination] {h in HOURS} <demand[destination,h]=col('h'||h)> ; 
    ...
    con extension_WTP {h in h_even}:
 	...
 	con extension_valves1 {h in h_even}:
        NumFlow["Clear","Tank_P",2*h]=NumFlow["Clear","Tank_P",2*h-1] ;
    con extension_valves2 {h in h_even}:
        NumFlow["Tank_E","Tank_N",2*h]=NumFlow["Tank_E","Tank_N",2*h-1] ;
    con extension_pump1 {h in h_even}:
		NumFlow["Clear","Tank_E",2*h-1]=NumFlow["Clear","Tank_E",2*h] ;
    con extension_pump2 {h in h_even}:
		NumFlow["Tank_P","Tank_N",2*h-1]=NumFlow["Tank_P","Tank_N",2*h] ;	
